...
{	
	...
	private function username($arg){
		if ( preg_match('/^@([a-zA-Z0-9_$&*#-]{0,})$/',is_array($arg) ? $arg[0]: $arg ) ) {
			...
			return  (object) ['valid' => false, 'error' => "The Username must contain the following format: '@youname' with letters and numbers!"] ;
		}
	}
	...
	//array(3) { [0]=> value [1]=> table [2]=> key }
		$db = new DB();
		if ( $db->exists($arg[1], $arg[2], $arg[0]) != false ) {
			return  (object) ['valid' => true] ;
		}
		else{		
			return  (object) [
			'valid' => false, 
			'error' => "Value $arg[2]:$arg[0] no exists in the Table ".ucfirst($arg[1]).'!' 
			] ;
		} 
	}
	...
	//array(3) { [0]=> value [1]=> table [2]=> key }
		...
		if ( $db->exists( $arg[1], $arg[2], $arg[0] ) == false ) {
			...
			return (object) [
			'valid' => false,
			 'error' =>  'Value already exists in the Table '.ucfirst($arg[1]).'!'
			 ] ;
		} 
	}
	...
	private function startwith($arg){
		...
		$arg[1] = '\\'.$arg[1];
		$select = "/[\s]*(".$arg[1].")/i";
		//echo $select; var_dump(preg_match( $select, $arg[0] )); exit;
